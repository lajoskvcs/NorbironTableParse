...
class Nodes {
    ...
    public Nodes(android.view.SurfaceView surfaceView) {
        ...
        neuronBox[6] = new NeuronBox(neuronSprite, 2 * 14, 64, 62, 15, nandIronProcCover, 0, 0);
...
public class NorbironSurfaceView extends android.view.SurfaceView implements Runnable {
    ...
    public void addBuildNode() {
        nodeBoxes.add((NeuronBox) nodes.get(6).clone());
    }

    public void queryNeuronBoxes() {
        nodeBoxes.clear();
        addBuildNode();
        ParseQuery<ParseObject> nodeQuery = ParseQuery.getQuery("Nodes");
        nodeQuery.whereEqualTo("createdBy", ParseUser.getCurrentUser());

        nodeQuery.findInBackground(new FindCallback<ParseObject>() {
            public void done(List<ParseObject> nodeList, ParseException e) {
                if(e == null) {
                    for(ParseObject node: nodeList) {
                        NeuronBox newNode = (NeuronBox) nodes.get(node.getInt("type")).clone();
                        newNode.setPk(node.getObjectId());
                        newNode.setXY(node.getInt("x"),node.getInt("y"));

                        nodeBoxes.add(newNode);
                    }
                }
            }
        });
